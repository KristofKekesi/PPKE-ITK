...
void berakod(Kontener kontener, Hajo& hajo) {
    if (hajo.kapacitas > hajo.rakter.size())
        hajo.rakter.push_back(kontener);
}

double arukirak(string tipus, double mennyiseg, Hajo& hajo) {
    for (Kontener& kontener : hajo.rakter) {
        if (kontener.fajta == tipus) {
            if (mennyiseg < kontener.toltottseg) {
                kontener.toltottseg -= mennyiseg;
                return 0;
            } else {
                mennyiseg -= kontener.toltottseg;
                kontener.toltottseg = 0;
            }
        }
    }
    return mennyiseg;
