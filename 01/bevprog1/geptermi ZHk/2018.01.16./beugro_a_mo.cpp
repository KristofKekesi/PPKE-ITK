...
void csatol(Vonat &vonat, Szerelveny szerelveny) {
    if (vonat.maximalisvontatmany > vonat.szerelvenyek.size())
        vonat.szerelvenyek.push_back(szerelveny);
}

double rakomany(Vonat &vonat, string tipus, double mennyiseg) {
    for (Szerelveny& szerelveny : vonat.szerelvenyek) {
        if (szerelveny.tipus == tipus) {
            double mennyi_fer_fel = szerelveny.maxkapacitas - szerelveny.kihasznaltsag;
            if (mennyi_fer_fel > 0) {
                if (mennyi_fer_fel > mennyiseg) {
                    szerelveny.kihasznaltsag += mennyiseg;
                    return 0;
                } else {
                    mennyiseg -= mennyi_fer_fel;
                    szerelveny.kihasznaltsag = szerelveny.maxkapacitas;
                }
            }
        }
    }
...
int main() {
    ...
    ellenoriz( sz1.tipus.compare( "szemely kocsi" ) == 0 && sz1.maxkapacitas+sz2.maxkapacitas+sz3.maxkapacitas-80 == 60000 && sqrt(sz2.azonosito)==121 && sz3.kihasznaltsag - 345.7 < 1e-4 );
